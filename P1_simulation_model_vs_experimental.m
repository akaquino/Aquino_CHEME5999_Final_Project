%% CHEME 5999 Simulation model vs experimental

clear all
close all


global  x1_o x2_o x3_o x4_o Eo kcat Km1 Km2 n n_EX kcat_EX Km1_EX Km2_EX 
...
legend('Glycan (dUG/dt), model', 'Polypeptide(dP/dt), model','Cofactor(dM/dt), model','Glycoprotein (dGP/dt), model','Glycan (dUG/dt), experimental', 'Polypeptide(dP/dt), experimental','Cofactor(dM/dt), experimental','Glycoprotein (dGP/dt), experimental'); 
...
global   x1_o x2_o x3_o n_EX Eo kcat_EX Km1_EX Km2_EX 

% enzyme activity dependency on cofactor w/ hill fxn  
theta = ((Eo*y(3))^n_EX)/(50*Eo*y(3) + (Eo*y(3))^n_EX); 

% define odes 
func1= (x1_o*F1)/(kcat_EX*V) -  (y(1)*F4)/(kcat_EX*V) - theta*(y(1)/(Km1_EX + y(1)))*(y(2)/(Km2_EX + y(2))); % dUG/dt
func2= (x2_o*F2)/(kcat_EX*V) -  (y(2)*F4)/(kcat_EX*V) - theta*(y(1)/(Km1_EX + y(1)))*(y(2)/(Km2_EX + y(2))); %dP/dt
func3 = (x3_o*F3)/(kcat_EX*V) - (y(3)*F4)/(kcat_EX*V) ; %dMn2+/dt
func4 = theta*(y(1)/(Km1_EX + y(1)))*(y(2)/(Km2_EX + y(2))) - (y(4)*F4)/(kcat_EX*V) ;%dGP/dt 
