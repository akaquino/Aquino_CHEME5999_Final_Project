...
global  x1_o x2_o x3_o x4_o Eo kcat Km1 Km2 n n_SA kcat_SA Km1_SA Km2_SA 
...
legend('Glycan (dUG/dt), model', 'Polypeptide(dP/dt), model','Cofactor(dM/dt), model','Glycoprotein (dGP/dt), model','Glycan (dUG/dt), SA', 'Polypeptide(dP/dt), SA','Cofactor(dM/dt), SA','Glycoprotein (dGP/dt), SA'); 
...
global   x1_o x2_o x3_o n_SA Eo kcat_SA Km1_SA Km2_SA 

% enzyme activity dependency on cofactor w/ hill fxn  
theta = ((Eo*y(3))^n_SA)/(50*Eo*y(3) + (Eo*y(3))^n_SA); 

% define odes 
func1= (x1_o*F1)/(kcat_SA*V) -  (y(1)*F4)/(kcat_SA*V) - theta*(y(1)/(Km1_SA + y(1)))*(y(2)/(Km2_SA + y(2))); % dUG/dt
func2= (x2_o*F2)/(kcat_SA*V) -  (y(2)*F4)/(kcat_SA*V) - theta*(y(1)/(Km1_SA + y(1)))*(y(2)/(Km2_SA + y(2))); %dP/dt
func3 = (x3_o*F3)/(kcat_SA*V) - (y(3)*F4)/(kcat_SA*V) ; %dMn2+/dt
func4 = theta*(y(1)/(Km1_SA + y(1)))*(y(2)/(Km2_SA + y(2))) - (y(4)*F4)/(kcat_SA*V) ;%dGP/dt 
